...
# Match the timeout used in the workflow files for graceful exit
WORKFLOW_TIMEOUT_SECONDS = 53 * 60

# --- Helper functions for DB and timeout ---
...
def bulk_upsert_to_worker_db(db_path, configs_data):
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    # Using UPSERT for cases where the same config might be re-tested
    cursor.executemany('''INSERT INTO configs (config, source_url, country_code, speed_kbps, last_tested) 
                          VALUES (?, ?, ?, ?, ?) 
...
def parse_proxy_uri_to_xray_json(uri: str):
    try:
        if uri.startswith("vless://"):
            ...
            # The JSON part is base64 encoded
            encoded_part = uri.replace("vmess://", "").strip()
            # Fix padding issues
            padding = len(encoded_part) % 4
            if padding: encoded_part += "=" * (4 - padding)
            decoded_json_str = base64.b64decode(encoded_part).decode('utf-8', errors='ignore')
            ...
            parsed = urlparse(uri); params = parse_qs(parsed.query)
            if not parsed.username: return None # Password is required
            ...
            if '@' in uri_no_fragment:
                # Standard format: ss://<user_info>@<host>:<port>
                parsed = urlparse(uri_no_fragment)
                user_info_part = unquote(parsed.username)
                padding = len(user_info_part) % 4
                if padding: user_info_part += "=" * (4 - padding)
                decoded_user_info = base64.urlsafe_b64decode(user_info_part).decode('utf-8', errors='ignore')
                method, password = decoded_user_info.split(":", 1)
                hostname, port = parsed.hostname, parsed.port
            else:
                # Legacy format: ss://<base64_encoded_part>
...
async def test_proxy_speed(proxy_config: str, port: int) -> float:
    """Tests a single proxy's download speed using xray and curl."""
    ...
    try:
        ...
        if proc.returncode == 0 and stdout:
            try:
                ...
                return speed_bytes_per_sec / 1024 # Return speed in KB/s
...
async def process_batch(batch, reader, start_port):
    """Processes a batch of configs for speed testing concurrently."""
    tasks = [asyncio.wait_for(test_proxy_speed(conn, start_port + i), timeout=TASK_TIMEOUT) for i, conn in enumerate(batch)]
    results = await asyncio.gather(*tasks, return_exceptions=True)
    
    successful_in_batch = []
    now = datetime.utcnow().isoformat()
    for conn, result in zip(batch, results):
        if isinstance(result, float) and result > 1: # Speed > 1 KB/s
            speed_kbps = result
...
async def main(input_path, db_path):
    """Main function to run the speed testing worker."""
