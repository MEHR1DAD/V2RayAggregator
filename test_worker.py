...
def parse_proxy_uri_to_xray_json(uri: str):
    ...
    try:
        # The parsers from the last successful version...
...
async def test_proxy(proxy_config: str, port: int) -> float:
...
async def main(input_path, db_path):
    ...
    print(f"--- Worker starting tests on {len(connections_to_test)} configs ---")
    start_port = 10900
    for i in range(0, len(connections_to_test), BATCH_SIZE):
        if is_approaching_timeout():
            print("⏰ Approaching workflow timeout. Stopping for this worker.")
            break
